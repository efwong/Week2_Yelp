...
class YelpClient: BDBOAuth1RequestOperationManager {
    ...
    func searchWithTerm(_ term: String, completion: @escaping ([Business]?, Int, Error?) -> Void) -> AFHTTPRequestOperation {
        return searchWithTerm(term, sort: nil, categories: nil, distance: nil, deals: nil, limit: nil, offset: nil, completion: completion)
    }
    
    func searchWithTerm(_ term: String, sort: YelpSortMode?, categories: [String]?, distance: Double?, deals: Bool?, limit: Int?, offset: Int?, completion: @escaping ([Business]?, Int, Error?) -> Void) -> AFHTTPRequestOperation {
        ...
        return self.get("search", parameters: parameters,
                        success: { (operation: AFHTTPRequestOperation, response: Any) -> Void in
                            if let response = response as? [String: Any]{
                                ...
                                if dictionaries != nil {
                                    completion(Business.businesses(array: dictionaries!), totalBusinesses, nil)
                                }
                            }
                        },
